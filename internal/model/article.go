...
// GetFilePathById 根据文章id获取文章的图片路径
func GetFilePathById(db *gorm.DB, id int) (path string, err error) {
	result := db.Model(&Article{}).Select("img").Where("id = ? and is_delete = 0", id).First(&path)
	if result.Error != nil {
		return "", result.Error
	}
