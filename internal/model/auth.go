...
type UserAuth struct {
	Model
	Username      string     `json:"username" gorm:"unique;type:varchar(50)"`           // 用户名，唯一，最大长度为50
	Password      string     `json:"-" gorm:"type:varchar(100)"`                        // 密码，最大长度为100，不会被JSON序列化
	LoginType     int        `json:"login_type" gorm:"type:tinyint(1);comment:登录类型"`    // 登录类型，Tinyint 类型，表示不同的登录方式（例如：用户名/密码、第三方登录等）
	IpAddress     string     `json:"ip_address" gorm:"type:varchar(20);comment:登录ip地址"` // 登录ip地址，最大长度为20
	IpSource      string     `json:"ip_source" gorm:"type:varchar(50);comment:登录ip来源"`  // IP来源，最大长度为50
	LastLoginTime *time.Time `json:"last_login_time"`                                   // 最后登录时间
	IsDisable     bool       `json:"is_disable"`                                        // 是否禁用
	IsSuper       bool       `json:"is_super"`                                          // 是否是超级管理员
	UserInfoId    int        `json:"user_info_id"`                                      // 用户信息表id
	UserInfo      *UserInfo  `json:"user_info"`                                         // 用户信息
	Roles         []*Role    `json:"roles" gorm:"many2many:user_auth_role"`             //
}

// MarshalBinary 将结构体序列化为json二进制数据
func (u *UserAuth) MarshalBinary() ([]byte, error) {
...
type Role struct {
	Model
	Name      string `json:"name" gorm:"unique"`  // 角色名，唯一
	Label     string `json:"label" gorm:"unique"` // 角色标签，唯一
	IsDisable bool   `json:"is_disable"`          // 是否禁用
...
type Resource struct {
	Model
	Name      string  `json:"name" gorm:"unique;type:varchar(50)"`    //资源名称，最大长度为50，唯一
	ParentId  int     `json:"parent_id"`                              //父级资源ID,表示资源之间的层级关系
	Url       string  `json:"url" gorm:"type:varchar(255)"`           //资源的URL，最大长度为255
	Method    string  `json:"request_method" gorm:"type:varchar(10)"` //资源的请求方法，例如 GET, POST, PUT, DELETE 等
	Anonymous bool    `json:"is_anonymous"`                           //是否匿名访问，表示该资源是否需要登录才能访问
	Roles     []*Role `json:"roles" gorm:"many2many:role_resource"`   //角色关联的资源，表示资源与角色之间的多对多关系
}
...
type Menu struct {
	Model
	ParentId     int    `json:"parent_id"`                                                  //父级菜单ID父菜单ID，用于标识菜单的层级关系
	Name         string `json:"name" gorm:"uniqueIndex:idx_name_and_path;type:varchar(20)"` //菜单名称,唯一引索
	Path         string `json:"path" gorm:"uniqueIndex:idx_name_and_path;type:varchar(50)"` //菜单路由地址,唯一引索
	Component    string `json:"component" gorm:"type:varchar(50)"`                          //菜单组件路径
	Icon         string `json:"icon" gorm:"type:varchar(50)"`                               //菜单图标
	OrderNum     int8   `json:"order_num"`                                                  //菜单排序
	Redirect     string `json:"redirect" gorm:"type:varchar(50)"`                           //菜单重定向地址
	Catalogue    bool   `json:"is_catalogue"`                                               //是否为目录，目录项不展开子菜单
	Hidden       bool   `json:"is_hidden"`                                                  //是否隐藏菜单,隐藏则不显示在菜单栏
	KeepAlive    bool   `json:"keep_alive"`                                                 //是否缓存该菜单
	External     bool   `json:"is_external"`                                                //是否为外链菜单
	ExternalLink string `json:"external_link" gorm:"type:varchar(255)"`                     //外链地址

	Roles []*Role `json:"roles" gorm:"many2many:role_menu"` //菜单关联的角色，表示与角色的多对多关系
}

// UserAuthRole 表示用户认证与角色之间的关联关系信息,作为 GORM 的中间表模型，在数据库中存储用户和角色的绑定关系，实现权限管理中的用户角色分配功能。
type UserAuthRole struct {
	UserAuthId int `gorm:"primaryKey;uniqueIndex:idx_user_auth_role"` //代表用户认证的唯一标识（外键）
	RoleId     int `gorm:"primaryKey;uniqueIndex:idx_user_auth_role"` //代表角色的唯一标识（外键）
}
...
func GetMenuListByUserId(db *gorm.DB, id int) (menus []Menu, err error) {
	var userAuth UserAuth
	result := db.Model(&UserAuth{Model: Model{ID: id}}).
