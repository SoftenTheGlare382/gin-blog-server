...
type UserInfo struct {
	Model
	Email    string `json:"email" gorm:"type:varchar(30)"`                    // 邮箱最大长度30字符，保存用户的电子邮件地址
	Nickname string `json:"nickname" gorm:"unique;type:varchar(30);not null"` // 昵称最大长度30字符，保存用户的昵称
	Avatar   string `json:"avatar" gorm:"type:varchar(1024);not null"`        // 头像最大长度1024字符，保存用户的头像
	Intro    string `json:"intro" gorm:"type:varchar(255)"`                   // 简介最大长度255字符，保存用户的简介
	Website  string `json:"website" gorm:"type:varchar(255)"`                 // 网站最大长度255字符，保存用户的网站等链接
}

// UserInfoVO 返回前端的用户信息
...
func UpdateUserLoginInfo(db *gorm.DB, id int, ipAddress, ipSource string) error {
	...
	result := db.Where("id", id).First(userAuth)
	return result.Error
}
...
func UpdateUserInfo(db *gorm.DB, id int, nickname, avatar, intro, website string) error {
	...
	//GORM 会根据 userInfo 的主键（ID）找到对应的记录，并仅更新之前通过 Select 指定的字段。
