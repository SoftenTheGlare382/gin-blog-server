...
func (*Menu) GetUserMenu(c *gin.Context) {
	...
	if auth.IsSuper { // 超级管理员
		menus, err = model.GetAllMenuList(db)
	} else {
		menus, err = model.GetMenuListByUserId(GetDB(c), auth.ID) // 普通用户todo:GetDB(c)
	}
...
// menus2MenuVos 构建菜单列表的树形数据结构, []Menu => []MenuVo
...
// getChildrenMap key 是菜单 ID, value 是该菜单对应的子菜单列表
...
// getFirstLevelMenus 筛选出一级菜单 (parentId == 0 的菜单)
func getFirstLevelMenus(menusList []model.Menu) []model.Menu {
...
func (*Menu) Delete(c *gin.Context) {
	...
	//检查删除的菜单是否被角色使用
	if use, _ := model.CheckMenuInUse(db, menuId); use {
...
func (*Menu) GetOption(c *gin.Context) {
	...
	for _, menu := range menus2MenuVos(menus) {
		...
		for _, child := range menu.Children {
			option.Children = append(option.Children, TreeOptionVO{
				ID:    child.ID,
				Label: child.Name,
			})
		}
