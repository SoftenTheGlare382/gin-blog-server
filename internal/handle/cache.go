...
func addConfigCache(rdb *redis.Client, Config map[string]string) error {
	return rdb.HSet(rctx, global.CONFIG, Config).Err()
}

// removeConfigCache 删除 Redis 中的博客配置缓存
func removeConfigCache(rdb *redis.Client) error {
	return rdb.Del(rctx, global.CONFIG).Err()
}

// 从 Redis 中获取博客配置缓存
// rdb.HGetAll 如果不存在 key, 不会返回 redis.Nil 错误, 而是返回空 map，调用 .Result() 会阻塞当前协程，直到 Redis 返回结果，并将结果转换为 (map[string]string, error) 类型。
