...
func (*ipUtil) GetIpAddress(c *gin.Context) (ipAddress string) {
	...
	if ipAddress == "" || len(ipAddress) == 0 || strings.EqualFold("unknown", ipAddress) {
		ipAddress = c.Request.Header.Get("Proxy-Client-IP") // "ip:port"
	}
	// WL-Proxy-Client-IP: WebLogic 服务代理
	if ipAddress == "" || len(ipAddress) == 0 || strings.EqualFold("unknown", ipAddress) {
		// WL-Proxy-Client-IP: WebLogic 服务代理
		ipAddress = c.Request.Header.Get("WL-Proxy-Client-IP") // "ip:port"
	}
	...
	// 如果 ipAddress 不为空，并且长度超过 15 个字符（说明可能是 IPv4 或包含逗号的内容）
	if ipAddress != "" && len(ipAddress) > 15 {
		// 检查 IP 地址字符串中是否包含逗号 ","
		// 例如：X-Forwarded-For 可能返回多个 IP，格式为 "client_ip,proxy1,proxy2"
		if strings.Index(ipAddress, ",") > 0 {
			// 如果发现有逗号，则截取第一个逗号前的内容
			// 即只保留客户端真实的 IP 地址部分
