...
var (
	_codes    = map[int]struct{}{}   //注册过的业务码集合，防止重复注册
	_messages = make(map[int]string) //业务码对应的错误信息
)

// RegisterResult
//
//	@Description:注册一个响应码，不允许重复注册
//	@Param	code	query	int		true	"错误码"
//	@Param	msg		path	string	true	"错误信息"
//	@return	Result 响应码结构体对象
func RegisterResult(code int, msg string) Result {
	if _, ok := _codes[code]; ok {
		panic(fmt.Sprintf("code %d is already registered", code))
	}
	if msg == "" {
		panic(fmt.Sprintf("code %d message can not be empty", code))
	}
...
var (
	...
	ErrNewPassword  = RegisterResult(1011, "新密码不能与旧密码相同")
	...
	ErrFileOpen    = RegisterResult(9102, "文件打开失败")
	ErrFileInfo    = RegisterResult(9103, "文件信息获取失败")
	ErrParseRange  = RegisterResult(9104, "文件头解析失败")
